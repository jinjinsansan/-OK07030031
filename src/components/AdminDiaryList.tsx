...
const AdminDiaryList: React.FC<AdminDiaryListProps> = ({
  ...
  const visibleEntries = entries.filter((e) => {
    // 公開フラグ
    const visible =
      e.is_visible_to_user === true ||
      e.is_visible_to_user === 'true' ||
      e.is_visible_to_user === 1;

    // 同期状態（null / '' / 'supabase'）
    const s = (e.syncStatus ?? '').trim();
    const synced = s === '' || s === 'supabase';
  ...
  const getEmotionColor = (emotion: string) => {
    ...
    return colorMap[emotion.trim()] || 'bg-white';
  };
  ...
  const getEmotionBorderColor = (emotion: string) => {
    ...
    return colorMap[emotion.trim()] || 'border-gray-200';
  };
  ...
  const getEmotionTextColor = (emotion: string) => {
    ...
    return colorMap[emotion.trim()] || 'text-gray-800';
  };
  ...
  return (
    <div className="space-y-4">
      {visibleEntries.map((entry) => {
        // デバッグ: 最終的に残った行を確認
        console.log('[visible]', entry.id);
        
        return (
          <div 
            key={entry.id}
            className={`border ${getEmotionBorderColor(entry.emotion)} rounded-lg p-4 hover:shadow-md transition-shadow ${getEmotionColor(entry.emotion.trim()) ?? 'bg-white'}`}
          >
