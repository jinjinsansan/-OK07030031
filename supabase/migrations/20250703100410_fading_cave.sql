...
-- 2. 新しいRLSポリシーを作成
CREATE POLICY "diary_entries_access_policy_final" ON diary_entries
  FOR ALL 
  TO authenticated
  USING (true)
  WITH CHECK (true);
...
-- 5. コメント
COMMENT ON POLICY "diary_entries_access_policy_final" ON diary_entries IS '認証済みユーザーが全ての日記エントリーにアクセスできるようにするポリシー';