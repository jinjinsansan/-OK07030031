/*
  ...
  1. Changes
    - Create a new RLS policy that allows authenticated users to access all diary entries
    - Fix NULL values in diary entries
    - Add a trigger to preserve user_id during updates
    - Optimize indexes for better search performance

  2. Purpose
    - Fix the issue where diary entries are not showing up in the admin panel's diary and search tabs
    - Ensure data integrity and consistency
    - Improve search performance
*/

-- 1. Create a new RLS policy that allows authenticated users to access all diary entries
CREATE POLICY "admin_panel_display_policy" ON diary_entries
  FOR ALL 
  TO authenticated
  USING (true)
  WITH CHECK (true);
...
COMMENT ON POLICY "admin_panel_display_policy" ON diary_entries IS 'Allows authenticated users to access all diary entries';